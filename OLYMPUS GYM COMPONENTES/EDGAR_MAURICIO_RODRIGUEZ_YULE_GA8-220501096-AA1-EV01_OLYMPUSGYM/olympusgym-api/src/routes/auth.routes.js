...
import { nextId, persistStore, publicUser, store } from '../data/store.js';
import { hashPassword, issueToken, verifyPassword } from '../utils/security.js';
...
router.post('/register', (req, res, next) => {
  try {
    ...
    persistStore();
    res.status(201).json({ ...publicUser(user), token: issueToken(user) });
  } catch (error) {
    next(error);
  }
});
...
router.post('/login', (req, res, next) => {
  try {
    ...
    res.status(200).json({ ...publicUser(user), token: issueToken(user) });
  } catch (error) {
    next(error);
  }
});
